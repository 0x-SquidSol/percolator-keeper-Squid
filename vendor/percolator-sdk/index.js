...
function buildLayoutV1M2(maxAccounts) {
  ...
  return {
    ...
    // 192 — V1M2 uses 312-byte accounts (same struct as V_ADL)
    hasInsuranceIsolation: true,
    engineInsuranceIsolatedOff: 48,
    engineInsuranceIsolationBpsOff: 64
  };
  ...
  return {
    ...
    dexPool: (() => {
      ...
      if (configLen >= DEX_POOL_REL_OFF + 32 && data.length >= configOff + DEX_POOL_REL_OFF + 32) {
        const bytes = data.subarray(configOff + DEX_POOL_REL_OFF, configOff + DEX_POOL_REL_OFF + 32);
        if (bytes.some((b) => b !== 0)) return new PublicKey3(bytes);
      }
      return null;
    })()
  };
...
function isAccountUsed(data, idx) {
  ...
  if (data.length < base + word * 8 + 8) {
    console.warn(
      `[isAccountUsed] Bitmap read would exceed buffer bounds: base=${base}, word=${word}, dataLen=${data.length}`
    );
    return false;
  }
  const bit = idx % 64;
...
async function discoverMarketsViaApi(connection, programId, apiBaseUrl, options = {}) {
  ...
  for (const entry of apiMarkets) {
    ...
    try {
      addresses.push(new PublicKey5(entry.slab_address));
    } catch {
      console.warn(
...
async function discoverMarketsViaStaticBundle(connection, programId, entries, options = {}) {
  ...
  for (const entry of entries) {
    ...
    try {
      addresses.push(new PublicKey5(entry.slabAddress));
    } catch {
      console.warn(
...
function parsePumpSwapPool(poolAddress, data) {
  ...
  return {
    dexType: "pumpswap",
    poolAddress,
    baseMint: new PublicKey6(data.slice(35, 67)),
    quoteMint: new PublicKey6(data.slice(67, 99)),
    baseVault: new PublicKey6(data.slice(131, 163)),
    quoteVault: new PublicKey6(data.slice(163, 195))
  };
}
...
function parseRaydiumClmmPool(poolAddress, data) {
  ...
  return {
    ...
    poolAddress,
    baseMint: new PublicKey6(data.slice(73, 105)),
    quoteMint: new PublicKey6(data.slice(105, 137))
  };
}
...
function parseMeteoraPool(poolAddress, data) {
  ...
  return {
    ...
    poolAddress,
    baseMint: new PublicKey6(data.slice(81, 113)),
    quoteMint: new PublicKey6(data.slice(113, 145))
  };
}
...
var TOKEN_2022_PROGRAM_ID = new PublicKey7(
...
// src/solana/stake.ts
import { PublicKey as PublicKey9, SystemProgram as SystemProgram2, SYSVAR_RENT_PUBKEY as SYSVAR_RENT_PUBKEY2, SYSVAR_CLOCK_PUBKEY as SYSVAR_CLOCK_PUBKEY2 } from "@solana/web3.js";
...
function getProgramId(network) {
  ...
  return new PublicKey8(programId);
}
...
function getStakeProgramId(network) {
  ...
  return new PublicKey9(id);
}
var STAKE_PROGRAM_ID = new PublicKey9(STAKE_PROGRAM_IDS.devnet);
var STAKE_IX = {
  InitPool: 0,
  Deposit: 1,
  Withdraw: 2,
...
function decodeStakePool(data) {
  ...
  off += 1;
  off += 4;
  const slab = new PublicKey9(bytes.subarray(off, off + 32));
  off += 32;
  const admin = new PublicKey9(bytes.subarray(off, off + 32));
  off += 32;
  const collateralMint = new PublicKey9(bytes.subarray(off, off + 32));
  off += 32;
  const lpMint = new PublicKey9(bytes.subarray(off, off + 32));
  off += 32;
  const vault = new PublicKey9(bytes.subarray(off, off + 32));
  off += 32;
  ...
  off += 8;
  const percolatorProgram = new PublicKey9(bytes.subarray(off, off + 32));
  off += 32;
...
async function checkRpcHealth(endpoint, timeoutMs = 5e3) {
  ...
  try {
    const slot = await Promise.race([
      ...
      rejectAfter(timeoutMs, `Health probe timed out after ${timeoutMs}ms`)
    ]);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        const result = await Promise.race([
          fn(ep.connection),
          rejectAfter(this.requestTimeoutMs, `RPC request timed out after ${this.requestTimeoutMs}ms (${ep.label})`)
        ]);
        ep.failures = 0;
        ep.healthy = true;
        return result;
...
import { PublicKey as PublicKey12, Transaction as Transaction2 } from "@solana/web3.js";
var LIGHTHOUSE_PROGRAM_ID = new PublicKey12(
...
function stripLighthouseInstructions(instructions) {
  return instructions.filter((ix) => !isLighthouseInstruction(ix));
}
function stripLighthouseFromTransaction(transaction) {
...
function computeFeeSplit(totalFee, config) {
  ...
  if (totalBps > 10000n) {
    throw new Error(
      `Fee split exceeds 100%: lpBps=${config.lpBps} + protocolBps=${config.protocolBps} + creatorBps=${config.creatorBps} = ${totalBps} > 10000`
    );
  }
